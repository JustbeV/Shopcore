...
export default function ProductForm({ storeId, categories, product }: Props) {
  ...
  const { data, setData, post, put, transform, processing, errors } = useForm({
    ...
    base_price_cents: product ? (product.base_price_cents / 100).toString() : '0',
    currency: product?.currency ?? 'USD',
    category_ids: product?.categories?.map((c) => c.id) ?? ([] as string[]),
  });

  // Transform data payload right before submit
  transform((formData) => ({
    ...formData,
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setData('base_price_cents', e.target.value)}
